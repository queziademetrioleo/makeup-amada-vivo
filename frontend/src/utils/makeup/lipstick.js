...
export function drawLipstick(ctx, landmarks, width, height, config) {
    ...
    const path = [...upper, ...lower.slice(1, -1)];
    ...
    if (config.glossy) {
        ...
        const lipHeight = Math.abs(landmarks[LANDMARK_GROUPS.lipsOuterLower[5]].y * height - hy);
        compositeLayer(ctx, width, height, {
            draw: (oc) => {
...
export function drawLipLiner(ctx, landmarks, width, height, color, opacity) {
    const upper = groupToPixels(LANDMARK_GROUPS.lipsOuterUpper, landmarks, width, height);
    const lower = groupToPixels(LANDMARK_GROUPS.lipsOuterLower, landmarks, width, height);
    const path = [...upper, ...lower.slice(1, -1)];
    compositeLayer(ctx, width, height, {
