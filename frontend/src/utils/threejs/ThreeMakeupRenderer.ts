...
function lmW(lm: { x: number; y: number; z: number }, W: number, H: number): [number, number, number] {
...
function fanGeo(pts: [number, number, number][]): THREE.BufferGeometry {
...
function buildLipGeo(landmarks: NormalizedLandmarkList, W: number, H: number): THREE.BufferGeometry {
...
function buildFoundationGeo(landmarks: NormalizedLandmarkList, W: number, H: number): THREE.BufferGeometry {
  const v2 = (i: number) => { const [x, y] = lmW(landmarks[i], W, H); return new THREE.Vector2(x, y); };
  const expandHole = (pts: THREE.Vector2[], s: number): THREE.Vector2[] => {
...
function mergeTwo(a: THREE.BufferGeometry, b: THREE.BufferGeometry): THREE.BufferGeometry {
  ...
  const uvA  = a.getAttribute('uv')?.array as Float32Array | undefined;
  const uvB  = b.getAttribute('uv')?.array as Float32Array | undefined;
  ...
  if (uvA && uvB) {
    ...
    uv.set(uvA); uv.set(uvB, uvA.length);
...
function setUniforms(
  ...
  (mat.uniforms.u_color.value as THREE.Vector3).set(r * r, g * g, b * b); // sRGB→linear
...
export class ThreeMakeupRenderer {
  ...
  private mats: {
    lip:     THREE.ShaderMaterial;
    brow:    THREE.ShaderMaterial;
    blush:   THREE.ShaderMaterial;
    contour: THREE.ShaderMaterial;
    found:   THREE.ShaderMaterial;
    bg:      THREE.ShaderMaterial;
  };
  private W = 0;
  private H = 0;
  ...
  constructor(canvas: HTMLCanvasElement, video: HTMLVideoElement) {
    ...
    const mk = (mat: THREE.ShaderMaterial, order: number): THREE.Mesh => {
    ...
    this.meshes = {
      ...
      blushR: mk(this.mats.blush.clone() as THREE.ShaderMaterial, 3),
      browL:  mk(this.mats.brow,    4),
      browR:  mk(this.mats.brow.clone() as THREE.ShaderMaterial, 4),
    ...
    for (const mesh of [this.meshes.blushR, this.meshes.browR]) {
      (mesh.material as THREE.ShaderMaterial).uniforms.u_video.value = this.videoTex;
    }
  }
  ...
  private setGeo(mesh: THREE.Mesh, geo: THREE.BufferGeometry) {
  ...
  render(
    ...
    if (landmarks) {
      ...
      setUniforms(this.meshes.browR.material as THREE.ShaderMaterial, browEnabled, browColor, browOpacity);
      ...
      setUniforms(this.meshes.blushR.material as THREE.ShaderMaterial, blushEnabled, blushColor, blushOpacity);
      ...
      for (const mesh of Object.values(this.meshes)) mesh.visible = true;
    } else {
