...
export class WebGLMakeupRenderer {
    ...
    constructor(canvas) {
        ...
        this.gl = gl;
        const prog = link(gl, compile(gl, gl.VERTEX_SHADER, VERT_SRC), compile(gl, gl.FRAGMENT_SHADER, FRAG_SRC));
        this.prog = prog;
        gl.useProgram(prog);
        // Full-screen quad
        ...
        for (const u of uniforms)
            this.locs[u] = gl.getUniformLocation(prog, u);
        // Bind texture units
        ...
        // Create textures
        for (const [name, slot] of [['video', 0], ['lip', 1], ['brow', 2], ['aux', 3]]) {
            this.textures[name] = this.makeTexture(slot);
        }
    }
    makeTexture(slot) {
        const { gl } = this;
    ...
    u2f(name, x, y) { this.gl.uniform2f(this.locs[name], x, y); }
    u3f(name, v) { this.gl.uniform3f(this.locs[name], v[0], v[1], v[2]); }
    render(video, lipCanvas, browCanvas, auxCanvas, config, blushLUV, blushRUV, blushRad) {
        const { gl } = this;
