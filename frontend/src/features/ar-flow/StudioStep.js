...
function groupPx(lm, indices, w, h) {
    return indices.map((i) => lmPx(lm, i, w, h));
}
// ── Composite helper ─────────────────────────────────────────────────────────
function composite(ctx, w, h, blur, blend, alpha, draw) {
    const oc = new OffscreenCanvas(w, h);
    const oc2 = oc.getContext('2d');
    if (blur > 0)
        oc2.filter = `blur(${blur}px)`;
    draw(oc2);
    ctx.save();
...
function renderLipstick(ctx, lm, w, h, color, opacity) {
    // Outer lip boundary
    ...
    const lipOuter = [...upper, ...lower.slice(1, -1)];
    ...
    const mouthHole = [...innerUpper, ...innerLower.slice(1, -1)];
    function drawLip(oc) {
...
function renderBlush(ctx, lm, w, h, color, opacity) {
...
function renderFoundation(ctx, lm, w, h, color, opacity) {
...
function renderContour(ctx, lm, w, h, color, opacity) {
    const faceW = Math.abs(lm[454].x - lm[234].x) * w;
...
function renderBrows(ctx, lm, w, h, color, opacity) {
    const faceW = Math.abs(lm[454].x - lm[234].x) * w;
    ...
    for (const indices of [LANDMARK_GROUPS.leftBrow, LANDMARK_GROUPS.rightBrow]) {
        ...
        const poly = [
            ...
            ...[...pts].reverse().map(([x, y]) => [x, y + thick * 0.5]),
        ];
...
function LiveCanvas({ videoRef, faceLandmarkerRef, faceMeshReady, colorRef, productRef }) {
        ...
        if (fl && faceMeshReady) {
            ...
            try {
                ...
                if (lm) {
                    ...
                    const opacity = PRODUCT_CONFIG[product]?.defaultOpacity ?? 0.8;
                    if (product === 'batom')
                        renderLipstick(ctx, lm, vw, vh, color, opacity);
                    if (product === 'blush')
                        renderBlush(ctx, lm, vw, vh, color, opacity);
                    if (product === 'base')
                        renderFoundation(ctx, lm, vw, vh, color, opacity);
                    if (product === 'contour')
                        renderContour(ctx, lm, vw, vh, color, opacity);
                    if (product === 'brows')
                        renderBrows(ctx, lm, vw, vh, color, opacity);
                }
            }
    ...
    return (_jsx("canvas", { ref: canvasRef, className: "absolute inset-0 w-full h-full object-cover", 
        // Mirror for selfie view — internally unmirrored so landmarks align
        style: { transform: 'scaleX(-1)' } }));
}
// ── StudioStep ───────────────────────────────────────────────────────────────
export function StudioStep({ videoRef, faceLandmarkerRef, faceMeshReady }) {
    ...
    return (_jsxs(motion.div, { className: "relative w-full h-full flex flex-col overflow-hidden bg-black", initial: { opacity: 0 }, animate: { opacity: 1 }, children: [_jsxs("div", { className: "relative flex-1 min-h-0", children: [_jsx(LiveCanvas, { videoRef: videoRef, faceLandmarkerRef: faceLandmarkerRef, faceMeshReady: faceMeshReady, colorRef: colorRef, productRef: productRef }), _jsxs("div", { className: "absolute top-0 inset-x-0 flex items-center justify-between px-4 pt-10 pb-4 bg-gradient-to-b from-black/60 to-transparent", children: [_jsx("button", { onClick: reset, className: "w-9 h-9 rounded-full bg-black/50 backdrop-blur-sm flex items-center justify-center", children: _jsx("svg", { width: "18", height: "18", viewBox: "0 0 24 24", fill: "none", stroke: "white", strokeWidth: "2.5", children: _jsx("path", { d: "M19 12H5M12 5l-7 7 7 7" }) }) }), _jsx("div", { className: "px-3 py-1.5 rounded-full bg-black/50 backdrop-blur-sm border border-white/20", children: _jsxs("span", { className: "text-white/80 text-xs font-medium", children: [info.label, " \u00B7 AO VIVO"] }) }), _jsx("div", { className: "w-9 h-9" })] }), !faceMeshReady && (_jsx("div", { className: "absolute bottom-4 inset-x-0 flex justify-center", children: _jsxs("div", { className: "px-4 py-2 rounded-full bg-black/70 backdrop-blur-sm flex items-center gap-2", children: [_jsx("div", { className: "w-3 h-3 rounded-full border-2 border-white/30 border-t-white animate-spin" }), _jsx("p", { className: "text-white/60 text-xs", children: "Carregando IA..." })] }) }))] }), _jsxs("div", { className: "flex-shrink-0 bg-zinc-950 border-t border-white/8 px-4 pt-4 pb-8", children: [_jsxs("div", { className: "flex items-center justify-between mb-4", children: [_jsxs("div", { children: [_jsx("p", { className: "text-white font-semibold text-sm", children: info.label }), _jsx("p", { className: "text-white/40 text-xs", children: info.description })] }), _jsx("div", { className: "w-8 h-8 rounded-full ring-2 ring-white/30 shadow-lg", style: { background: selectedColor } })] }), _jsx("div", { className: "flex gap-3 overflow-x-auto pb-2 -mx-1 px-1", style: { scrollbarWidth: 'none' }, children: info.swatches.map((swatch) => {
                            const active = selectedColor === swatch.hex;
                            return (_jsxs("button", { onClick: () => setSelectedColor(swatch.hex), className: "flex-shrink-0 flex flex-col items-center gap-1.5", children: [_jsx("div", { className: `w-12 h-12 rounded-full transition-all duration-200 ${active
                                            ? 'ring-[3px] ring-white ring-offset-[3px] ring-offset-zinc-950 scale-110'
                                            : 'ring-1 ring-white/20 hover:scale-105'}`, style: { background: swatch.hex } }), _jsx("span", { className: "text-[10px] text-white/40 w-12 text-center leading-tight", children: swatch.name })] }, swatch.hex));
                        }) }), _jsx("button", { className: "w-full mt-4 py-3.5 rounded-2xl bg-rose-500 hover:bg-rose-400 transition-colors text-white font-semibold text-sm", children: "Adicionar ao carrinho" })] })] }));
}
